For use TFTGLCD Adapter you need
1. Replace files in Marlin (for Marlin-1) or Marlin-bugfix-2.0.x\Marlin directory with new from this directiry
   excluding Configuration.h - it for example only.
...
3. You need check section with "#elif ENABLED(TFTGLCD_ADAPTER)" in Conditionals_LCD.h (for Marlin-1) or
   src\inc\Conditionals_LCD.h (for Marlin-2) for right parameters for panel.
